...
- Pass control back to the .gemini/commands/apm-1-initiate-setup.toml logic.
- **Context Recovery:** When generating the Bootstrap Prompt, you must use the **EXACT TEMPLATE** from .gemini/commands/apm-1-initiate-setup.toml. If the template is degraded or missing from your context window, **READ .gemini/commands/apm-1-initiate-setup.toml** to retrieve it before generating the final artifact.

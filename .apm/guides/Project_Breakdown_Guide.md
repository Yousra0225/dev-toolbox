...
2. **If User skips Review:** Proceed directly to **Manager Bootstrap Creation**.
  - **CRITICAL:** You must generate the Bootstrap Prompt using the **EXACT TEMPLATE** defined in your initiation prompt .gemini/commands/apm-1-initiate-setup.toml.
  - **Context Recovery:** If you cannot retrieve the template word-for-word from your context, you must **READ** the .gemini/commands/apm-1-initiate-setup.toml file to refresh your memory before generating the prompt. Do not approximate the template.
